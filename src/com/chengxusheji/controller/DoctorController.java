...
import com.chengxusheji.service.DoctorAbsentService;
...
public class DoctorController extends BaseController {
    ...
    @Resource
    DoctorAbsentService doctorAbsentService;
    ...
    public void update(@PathVariable String doctorNumber, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        JSONObject jsonDoctor = doctor.getJsonObject();
        List<DoctorAbsent> doctorAbsents = doctorAbsentService.findAllByDoctorNumber(doctorNumber);
        List<String> dates = new ArrayList<>();
        for (DoctorAbsent d :
                doctorAbsents) {
            dates.add(simpleDateFormat.format(d.getAbsentDate()));
        }

        String[] absentDates = new String[dates.size()];
        String absentDateStr = "";
        for (int i = 0; i < dates.size(); i++) {
            absentDates[i] = dates.get(i);
        }
        for (String date :
                absentDates) {
            absentDateStr += date + ",";
        }
        absentDateStr = absentDateStr.substring(0,absentDateStr.length()-1);

        jsonDoctor.accumulate("absentDates",absentDateStr);
    ...
    public void update(@Validated Doctor doctor, BindingResult br,
        ...
        try {
            ...
            String absentDates = request.getParameter("absentDates");
            doctorAbsentService.removeByDoctorNumber(doctor.getDoctorNumber());

            String[] absentDateArr = absentDates.split(",");
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
            DoctorAbsent doctorAbsent = new DoctorAbsent();
            for(int i = 0;i < absentDateArr.length;i++){
                doctorAbsent.setId(0);
                doctorAbsent.setAbsentDate(simpleDateFormat.parse(absentDateArr[i]));
                doctorAbsent.setDoctorNumber(doctor.getDoctorNumber());
                doctorAbsentService.add(doctorAbsent);
            }
            message = "医生信息更新成功!";
            success = true;
            writeJsonResponse(response, success, message);
