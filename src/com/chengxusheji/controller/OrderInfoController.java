...
public class OrderInfoController extends BaseController {
    ...
    public void userAdd(OrderInfo orderInfo, BindingResult br,
        ...
        List<DoctorAbsent> doctorAbsents = doctorAbsentService.findAllByDoctorNumber(orderInfo.getDoctorObj().getDoctorNumber());
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
        List<String> absentDates = new ArrayList<>();
        for (DoctorAbsent d :
                doctorAbsents) {
            absentDates.add(simpleDateFormat.format(d.getAbsentDate()));
        }
        if (absentDates.contains(orderInfo.getOrderDate())) {
            message = "该日期医生不值班，请重新选择";
            writeJsonResponse(response, success, message);
            return;
        }
    ...
    /*前台用户按照查询条件分页查询自己的预约信息信息*/
    @RequestMapping(value = {"/userFrontlist"}, method = {RequestMethod.GET, RequestMethod.POST})
    public String userFrontlist(@ModelAttribute("userObj") UserInfo userObj, @ModelAttribute("doctorObj") Doctor doctorObj, String orderDate, String timeInterval, String telephone, String checkState, String orderTime, Integer currentPage, Model model, HttpServletRequest request, HttpSession session) throws Exception {
        if (currentPage == null || currentPage == 0) currentPage = 1;
        if (orderDate == null) orderDate = "";
        if (timeInterval == null) timeInterval = "";
        if (telephone == null) telephone = "";
